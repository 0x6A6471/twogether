defmodule HitchedWeb.AuthTest do
  use HitchedWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Hitched.Accounts
  alias HitchedWeb.Auth
  import Hitched.AccountsFixtures

  @remember_me_cookie "_hitched_web_user_remember_me"

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, HitchedWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "login_user/3" do
    ...
    test "clears everything previously stored in the session", %{conn: conn, user: user} do
      conn = conn |> put_session(:to_be_removed, "value") |> Auth.login_user(user)
      refute get_session(conn, :to_be_removed)
    end

    test "redirects to the configured path", %{conn: conn, user: user} do
      conn = conn |> put_session(:user_return_to, "/hello") |> Auth.login_user(user)
      assert redirected_to(conn) == "/hello"
    end

    test "writes a cookie if remember_me is configured", %{conn: conn, user: user} do
      conn = conn |> fetch_cookies() |> Auth.login_user(user, %{"remember_me" => "true"})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      HitchedWeb.Endpoint.subscribe(live_socket_id)
    ...
    test "works even if user is already logged out", %{conn: conn} do
      conn = conn |> fetch_cookies() |> Auth.logout_user()
      refute get_session(conn, :user_token)
      assert %{max_age: 0} = conn.resp_cookies[@remember_me_cookie]
      assert redirected_to(conn) == ~p"/"
    end
  ...
  describe "fetch_current_user/2" do
    ...
    test "authenticates user from cookies", %{conn: conn, user: user} do
      logged_in_conn =
        conn |> fetch_cookies() |> Auth.login_user(user, %{"remember_me" => "true"})
